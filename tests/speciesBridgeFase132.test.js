...
describe('Fase 13.2 — bellwave — novos mapeamentos da linha Dinomon', () => {
    ...
    describe('Mapeamentos diretos', () => {
        it('MON_011 (Dinomon, Comum) mapeia para bellwave — SPD/ATK 1.50, ENE/ATK 1.17 após offsets', () => {
            expect(resolveCanonSpeciesId('MON_011')).toBe('bellwave');
        });

        it('MON_011B (Guitarapitormon, Incomum) mapeia para bellwave — SPD/ATK 1.38, ENE/ATK 1.12 após offsets', () => {
            expect(resolveCanonSpeciesId('MON_011B')).toBe('bellwave');
        });

        it('MON_011C (TRockmon, Raro) mapeia para bellwave — SPD/ATK 1.18, ENE/ATK 1.00 após offsets', () => {
    ...
    describe('Contagem na tabela', () => {
        ...
        it('os 6 mapeamentos bellwave são exatamente MON_011/B/C e MON_027/B/C', () => {
            const bellwaveIds = Object.entries(RUNTIME_TO_CANON_SPECIES)
                .filter(([, v]) => v === 'bellwave')
                .map(([k]) => k)
                .sort();
            expect(bellwaveIds).toEqual([
                'MON_011', 'MON_011B', 'MON_011C',
                'MON_027', 'MON_027B', 'MON_027C',
            ]);
        });

        it('tabela total: 51 mapeamentos após Fase 13.2', () => {
...
describe('Fase 13.2 — wildpace — exclusões confirmadas (sem novos mapeamentos)', () => {
    ...
    it('MON_006 (Lobinho) continua sem mapeamento — estágio único', () => {
        expect(resolveCanonSpeciesId('MON_006')).toBeNull();
    });

    it('MON_012 (Luvursomon) continua sem mapeamento — ENE_off=4 (muito apertado)', () => {
...
describe('Fase 13.2 — bellwave offsets — linha Dinomon', () => {
    ...
    describe('MON_011 (Dinomon) — ATK:6, DEF:5, SPD:8, ENE:6, HP:27', () => {
    ...
    describe('MON_011B (Guitarapitormon) — ATK:8, DEF:6, SPD:10, ENE:8, HP:35', () => {
    ...
    describe('MON_011C (TRockmon) — ATK:11, DEF:9, SPD:12, ENE:10, HP:46', () => {
        const base = { hpMax: 46, atk: 11, def: 9, spd: 12, eneMax: 10 };
...
describe('Fase 13.2 — regressão — mapeamentos anteriores não afetados', () => {
    ...
    describe('Espécies MVP (shieldhorn, emberfang, moonquill, floracura, swiftclaw) intactas', () => {
        it('MON_010 → shieldhorn', () => { expect(resolveCanonSpeciesId('MON_010')).toBe('shieldhorn'); });
        it('MON_021 → emberfang', () => { expect(resolveCanonSpeciesId('MON_021')).toBe('emberfang'); });
        it('MON_003 → moonquill', () => { expect(resolveCanonSpeciesId('MON_003')).toBe('moonquill'); });
        it('MON_004 → floracura', () => { expect(resolveCanonSpeciesId('MON_004')).toBe('floracura'); });
        it('MON_013 → swiftclaw', () => { expect(resolveCanonSpeciesId('MON_013')).toBe('swiftclaw'); });
    });

    describe('Templates sem mapeamento continuam com null', () => {
        it('MON_001 (Cantapau) → null', () => { expect(resolveCanonSpeciesId('MON_001')).toBeNull(); });
        it('MON_005 (Garruncho) → null', () => { expect(resolveCanonSpeciesId('MON_005')).toBeNull(); });
        ...
        it('MON_006 (Lobinho) → null', () => { expect(resolveCanonSpeciesId('MON_006')).toBeNull(); });
    ...
    describe('Cobertura total do bridge pós-Fase 13.2', () => {
        it('51 mapeamentos totais (48 anteriores + 3 novos bellwave)', () => {
            expect(Object.keys(RUNTIME_TO_CANON_SPECIES)).toHaveLength(51);
        });
