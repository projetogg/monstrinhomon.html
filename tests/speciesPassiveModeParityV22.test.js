...
import { fireCombatEvent, ON_ATTACK, ON_HEAL_ITEM, ON_HIT, ON_SKILL_USED } from '../js/combat/combatEvents.js';
import { getActivePassiveIds } from '../js/canon/speciesPassives.js';
import { executeWildAttack, executeWildItemUse } from '../js/combat/wildActions.js';
import { executePlayerAttackGroup, executeUseItemGroup } from '../js/combat/groupActions.js';
import {
  getBuffModifiers,
...
function makeGroupDeps({ playerMonster, enemy, rolls, power = 7, inventory = {} }) {
  ...
  const deps = {
    ...
    helpers: {
      ...
      applyDamage: (target, damage) => { target.hp = Math.max(0, target.hp - damage); },
      handleVictoryRewards: () => {},
      openSwitchMonsterModal: () => {},
      getItemDef: () => ({ type: 'heal', name: 'Cura', emoji: '💚', heal_pct: 0.30, heal_min: 5 }),
    },
  };
...
describe('Passivas de espécie v2.2 — integração estrutural Wild e Group', () => {
  const wildBasic = segment(WILD_SOURCE, 'export function executeWildAttack', 'function applyEneRegen');
  const wildSkill = segment(WILD_SOURCE, 'export function executeWildSkill', 'export function executeWildCaptureAction');
  const wildItem = segment(WILD_SOURCE, 'export function executeWildItemUse', 'function handleVictory');
  const groupBasic = segment(GROUP_SOURCE, 'export function executePlayerAttackGroup', 'function findPlayerNeedingSwitch');
  const groupSkill = segment(GROUP_SOURCE, 'export function executePlayerSkillGroup', 'export function executeUseItemGroup');
  const groupItem = segment(GROUP_SOURCE, 'export function executeUseItemGroup', 'export function executeGroupUseItem');
  ...
  it('caracteriza o estágio divergente de aplicação de atkBonus', () => {
    expect(wildBasic).toContain('effectiveAtkForDamage = Math.max(1, effectiveAtkForDamage + atkPassive.atkBonus)');
    expect(groupBasic).toContain('dmg = Math.max(1, dmg + atkSpeciesPassive.atkBonus)');
  });
...
describe('Passivas de espécie v2.2 — efeitos observáveis por modo', () => {
  ...
  it('DRIFT: a ordem de shieldhorn e resistência de Guerreiro muda o dano em cenário de fronteira', () => {
    const attackerWild = () => makeMonster({ class: 'Mago', atk: 1, hp: 100, canonSpeciesId: null });
    const defenderWild = speciesId => makeEnemy({ class: 'Guerreiro', def: 8, canonSpeciesId: speciesId });

    const wildBase = runWildBasic({
      playerMonster: attackerWild(),
      enemy: defenderWild(null),
      d20A: 17,
      d20D: 10,
    });
    const wildShield = runWildBasic({
      playerMonster: attackerWild(),
      enemy: defenderWild('shieldhorn'),
      d20A: 17,
      d20D: 10,
    });
    const groupBase = runGroupBasic({
  ...
  it('PARITY: floracura concede +3 HP e consome o estado de primeira cura nos dois modos', () => {
    const wildMon = makeMonster({ class: 'Curandeiro', hp: 50, hpMax: 100, canonSpeciesId: 'floracura' });
    ...
    const wildPlayer = {
      id: 'p1', name: 'Player', class: 'Curandeiro', team: [wildMon], inventory: { HEAL: 1 },
    };
    ...
    const groupMon = makeMonster({ class: 'Curandeiro', hp: 50, hpMax: 100, canonSpeciesId: 'floracura' });
